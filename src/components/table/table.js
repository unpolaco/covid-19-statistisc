...
const SummaryTable = () => {
	...
	const prepareDataToRender = (lastData, compareData) => {
		...
		lastData.forEach((el, index) => {
			el.newConfirmed = assignIfPositive(
				el.confirmed - compareData[index].confirmed
			);
			...
			el.deathRate = assignIfPositive(
				+((el.deaths / el.confirmed) * 100).toFixed(1)
			);
		});
	...
	useEffect(() => {
		if (lastDayData.data && prevDayData.data) {
			const newData = prepareDataToRender(
				lastDayData.data.results,
				prevDayData.data.results
			);
			setData(newData);
		}
	...
	return (
		<section className={styles.section_table} id='table'>
							...
							<tr>
								{casesDetails.map((el, index) => {
									return (
										<th
											key={el.value[0]}
											onClick={() => sortOnClick(el)}
											className={`${styles.cursor_pointer} ${styles[el.class]}`}
										>
											{el.displayName}
								...
								return (
									<tr key={index}>
										{casesDetails.map((el, index) => {
											...
											return (
												<td className={styles[el.width]} key={el.value[0]}>
													{el.displayName.toLowerCase().includes('new') &&
														element[firstKey] > 0 && <span>+</span>}
													{(secondKey
														? element[firstKey][secondKey]
														: element[firstKey]) || ''}
													{el.displayName.toLowerCase().includes('rate') &&
														element[firstKey] > 0 && <span>%</span>}
												</td>
											);
										})}
